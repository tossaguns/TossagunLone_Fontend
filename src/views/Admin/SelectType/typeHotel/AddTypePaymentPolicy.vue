...
const fetch = async () => {
  try {
    const res = await axios.get('http://localhost:9999/HotelSleepGun/typePaymentPolicy/getAll')
    item.value = res.data
    currentPage.value = 1
  } catch (err) {
    console.error(err)
...
const createFacility = async () => {
  try {
    ...
    await axios.post('http://localhost:9999/HotelSleepGun/typePaymentPolicy/create', fd)
    alert('เพิ่มนโยบายการจ่ายเงินเรียบร้อยแล้ว')
    resetForm()
    fetch()
  } catch (err) {
    console.error(err)
...
const updateFacility = async () => {
  try {
    ...
    await axios.put(`http://localhost:9999/HotelSleepGun/typePaymentPolicy/update/${selectedId.value}`, fd)
    alert('อัปเดตนโยบายการจ่ายเงินเรียบร้อยแล้ว')
    resetForm()
    fetch()
  } catch (err) {
    console.error(err)
...
const deleteFacility = async (id) => {
  ...
  try {
    await axios.delete(`http://localhost:9999/HotelSleepGun/typePaymentPolicy/delete/${id}`)
    fetch()
  } catch (err) {
    console.error(err)
  }
