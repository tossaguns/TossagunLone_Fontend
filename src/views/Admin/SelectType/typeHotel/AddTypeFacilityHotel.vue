<template>
  ...
  <div>
    <div class="mb-4 text-lg font-bold text-lime-700">
      <p>3. สิ่งอำนวยความสะดวกของโรงเเรม</p>
    </div>
      ...
      <div class="px-4 py-6 ">
        ...
        <div>
          <div class="hidden md:block">
            <table class="min-w-full shadow-md">
              <thead>
                <tr class="bg-lime-700 text-white text-center text-sm">
              ...
              <tbody>
                <tr v-for="(item, index) in paginated" :key="item._id"
                  :class="index % 2 === 0 ? 'bg-white' : 'bg-lime-50'" class="border-t">
        ...
        <div class="mt-4 flex justify-center space-x-2 text-gray-400">
          <button class="" :disabled="currentPage === 1" @click="currentPage--">
            <p class="text-xs mr-3 hover:text-lime-900 hover:text-sm transition">ก่อนหน้า</p>
          </button>

          <button v-for="page in totalPages" :key="page" class="px-2 rounded-full "
            :class="page === currentPage ? 'bg-lime-700 text-white hover:bg-lime-600 transition' : 'bg-gray-100'"
            @click="goToPage(page)">
            {{ page }}
          </button>

          <button class="" :disabled="currentPage === totalPages" @click="currentPage++">
            <p class="text-xs ml-3 hover:text-lime-900 hover:text-sm transition">ถัดไป</p>
          </button>
        </div>
      </div>
    </div>
...
const createFacility = async () => {
  try {
    ...
    await axios.post('http://localhost:9999/HotelSleepGun/typeFacilityHotel/create', fd)
...
const updateFacility = async () => {
  try {
    ...
    await axios.put(`http://localhost:9999/HotelSleepGun/typeFacilityHotel/update/${selectedId.value}`, fd)
...
const deleteFacility = async (id) => {
  ...
  try {
    await axios.delete(`http://localhost:9999/HotelSleepGun/typeFacilityHotel/delete/${id}`)
    fetch()
  } catch (err) {
    console.error(err)
  }
