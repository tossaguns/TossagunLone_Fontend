...
const createFacility = async () => {
  try {
    ...
    await axios.post('http://localhost:9999/HotelSleepGun/typeHotelLocation/create', fd)
    alert('เพิ่มสถานที่ใกล้เคียงเรียบร้อยแล้ว')
    resetForm()
    fetch()
  } catch (err) {
    console.error(err)
...
const updateFacility = async () => {
  try {
    ...
    await axios.put(`http://localhost:9999/HotelSleepGun/typeHotelLocation/update/${selectedId.value}`, fd)
    alert('อัปเดตสถานที่ใกล้เคียงเรียบร้อยแล้ว')
    resetForm()
    fetch()
  } catch (err) {
    console.error(err)
...
const deleteFacility = async (id) => {
  ...
  try {
    await axios.delete(`http://localhost:9999/HotelSleepGun/typeHotelLocation/delete/${id}`)
    fetch()
  } catch (err) {
    console.error(err)
  }
