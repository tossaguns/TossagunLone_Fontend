<template>
  <!--================ TypeHotelFor ======================-->
  <div>
    <div class="mb-4 text-lg font-bold text-rose-700">
      <p>2.ประเภทห้องพัก</p>
    </div>
    ...
    <div class="max-w-5xl mx-auto border rounded-lg shadow-md">
      <div class="px-4 py-6 ">
        <form @submit.prevent="createFacility" class="space-y-4 mb-8">
          <div>
            <label class="font-semibold">ประเภทห้องพัก</label>
            <input v-model="addForm.name" type="text"
              class="border w-full py-2 px-2 rounded-md mt-1 border-stone-300 hover:border-rose-600"
          ...
          <div class="w-full  flex justify-center items-center flex-col space-y-4 pt-3">
            ...
            <div>
              <button type="submit" class="bg-rose-600 rounded-lg 
                        px-4 py-2 text-white hover:bg-rose-700 transition">เพิ่มประเภทห้องพัก</button>
            </div>
          </div>
        </form>
        ...
        <div v-if="isEditing" ref="editFormSection"
          class="mt-8 border-t max-w-2xl mx-auto rounded-lg border px-4 py-5 mb-8 shadow-lg bg-rose-50">
          <h2 class="text-xl font-semibold mb-2 text-rose-700 text-center">แก้ไขที่พักอนุญาตสำหรับ</h2>
          <form @submit.prevent="updateFacility" class="space-y-4">
            ...
            <div class="pt-10 space-x-2 flex justify-center">
              <button type="submit" class="bg-rose-400 rounded-lg 
                        px-4 py-2  hover:bg-rose-500 transition">อัปเดตข้อมูล</button>
        ...
        <div>
          <div class="hidden md:block">
            <table class="min-w-full shadow-md">
              <thead>
                <tr class="bg-rose-700 text-white text-center text-sm">
              ...
              <tbody>
                <tr v-for="(item, index) in paginated" :key="item._id"
                  :class="index % 2 === 0 ? 'bg-white' : 'bg-rose-50'" class="border-t">
        ...
        <div class="mt-4 flex justify-center space-x-2 text-gray-400">
          <button class="" :disabled="currentPage === 1" @click="currentPage--">
            <p class="text-xs mr-3 hover:text-rose-900 hover:text-sm transition">ก่อนหน้า</p>
          </button>

          <button v-for="page in totalPages" :key="page" class="px-2 rounded-full "
            :class="page === currentPage ? 'bg-rose-700 text-white hover:bg-rose-600 transition' : 'bg-gray-100'"
            @click="goToPage(page)">
            {{ page }}
          </button>

          <button class="" :disabled="currentPage === totalPages" @click="currentPage++">
            <p class="text-xs ml-3 hover:text-rose-900 hover:text-sm transition">ถัดไป</p>
          </button>
        </div>
      </div>
    </div>
...
const fetch = async () => {
  try {
    const res = await axios.get('http://localhost:9999/HotelSleepGun/typeRoom/getAll')
    item.value = res.data
    currentPage.value = 1
  } catch (err) {
    console.error(err)
...
const createFacility = async () => {
  try {
    ...
    await axios.post('http://localhost:9999/HotelSleepGun/typeRoom/create', fd)
    alert('เพิ่มประเภทห้องพักเรียบร้อยแล้ว')
    resetForm()
    fetch()
  } catch (err) {
    console.error(err)
...
const updateFacility = async () => {
  try {
    ...
    await axios.put(`http://localhost:9999/HotelSleepGun/typeRoom/update/${selectedId.value}`, fd)
    alert('อัปเดตประเภทห้องพักเรียบร้อยแล้ว')
    resetForm()
    fetch()
  } catch (err) {
    console.error(err)
...
const deleteFacility = async (id) => {
  ...
  try {
    await axios.delete(`http://localhost:9999/HotelSleepGun/typeRoom/delete/${id}`)
    fetch()
  } catch (err) {
    console.error(err)
  }
