...
const createFacility = async () => {
  try {
    ...
    await axios.post('http://localhost:9999/HotelSleepGun/typeHotelFor/create', fd)
    alert('เพิ่มการอนุญาติสำหรับที่พักเรียบร้อยแล้ว')
    resetForm()
    fetch()
  } catch (err) {
    console.error(err)
...
const updateFacility = async () => {
  try {
    ...
    await axios.put(`http://localhost:9999/HotelSleepGun/typeHotelFor/update/${selectedId.value}`, fd)
    alert('อัปเดตการอนุญาตสำหรับที่พักเรียบร้อยแล้ว')
    resetForm()
    fetch()
  } catch (err) {
    console.error(err)
...
const deleteFacility = async (id) => {
  ...
  try {
    await axios.delete(`http://localhost:9999/HotelSleepGun/typeHotelFor/delete/${id}`)
    fetch()
  } catch (err) {
    console.error(err)
  }
